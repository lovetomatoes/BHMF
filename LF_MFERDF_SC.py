...
# print(len(f_range)*len(l_range)*len(a_range) )
# exit(0)
...
for f_duty in f_range:
    for l_cut in l_range:
        print('M_1450_star form M_BH_star=2e9Ms',M1450_Lbol(L_M(2.e9,l_cut)))
        for a in a_range:
            i = i+1
        # # --------- Luminosity Function ---------
            Phi = np.zeros(N_lf)
            ...
            for ibin in range(N_lf):
                # #----------- Schechter lbd -----------
                x0 = kernelS_M1450(L_right[ibin], M_BH, l_cut)
                x1 = kernelS_M1450(L_left[ibin],  M_BH, l_cut)
