...
# ts = np.linspace(0.01,1,)*Dt
# np.arange(,)
t_life, d_fit = 100 ,  0.0001
t_life *= Myr
a = .5

Nt = Dt//t_life
print(Nt+1,Dt/t_life)
ls = np.random.gamma(shape=a, scale=l_cut, size=int(Nt+1))
print(a,l_cut,logM0)
ls[0] = .6
t_life = .01*t_Edd
print('ls[0]={0:e}\nM0={1:e}'.format(ls[0], M0M1(M1, ls[0], t_life, d_fit)))
# print('Newton solve:%e'%(Newton_method(M1, t_life, ls[0])))
exit(0)
for i in range(Nt):
