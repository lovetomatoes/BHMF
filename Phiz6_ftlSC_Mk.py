                        ...
                        for i_bsm in range(Nbsm):
                            ...
                            while Nt>=0:
                                ...
                                for ibin in range(N_mf):
                                    ...
                                    x0 = kernelS_MBH_Mk(M_BH[ibin], bin_right, t_life, f_duty, l_cut, k_fit)
                                    x1 = kernelS_MBH_Mk(M_BH[ibin], bin_left,  t_life, f_duty, l_cut, k_fit)
                                    x0[x0<0] = 0.; x1[x1<0] = 0. # let P(growth_ratio<1)=0, must! or not conserved!
                                    for i in range(len(x0)):
                                        assert x0[i]<= x1[i]
