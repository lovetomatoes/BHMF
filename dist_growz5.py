...
T = ascii.read(z6datapre+'LF_z5N3', guess=False, delimiter=' ') #  None has np.where(T['z_col']==-1)
Nsite = len(T) # N3 <-> 1e7
...
for flambda in [.11, .12, .13]:
    ...
    # MF
        ...
        plt.savefig(z5figpre+'/z'+str(z)+'f'+str(int(flambda*100))+'s'+str(int(sigma_fit*100))+'N'+str(int(np.log10(Nsite)-4))+'.png')
        ascii.write(T, z5datapre+'LF_z'+str(z)+'f'+str(int(flambda*100))+'s'+str(int(sigma_fit*100))+'N'+str(int(np.log10(Nsite)-4)),overwrite=True)
